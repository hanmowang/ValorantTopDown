...
public class Viper extends GameObject{
    private Handler handler;
    private GameObject player;
    private int HEALTH = 150;
    Random r = new Random();
    public Viper(float x, float y, ID id, Handler handler)
    {
        super(x,y,id);
    ...
    public void tick() {
        ...
        if(spawn==0)
        {
            handler.addObject(new ViperMolotov(r.nextInt(Game.WIDTH),r.nextInt(Game.HEIGHT),ID.ViperMolotov,handler));
        }
        spawn=r.nextInt(100);
        if(spawn==0)
        {
